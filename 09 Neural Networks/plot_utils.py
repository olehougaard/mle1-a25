...
def plot_probabilities(classifier, X, y, cmap, levels = 10, dim=0, ax=None, post_process = lambda x: x):
    if not ax:
        fig, ax = plt.subplots(1, 1)
    xs, ys =  X[:, 0], X[:, 1]
    ...
    Z = classifier.predict(np.c_[xx.ravel(), yy.ravel()])[:, dim]
    Z = post_process(Z)
    cset = ax.contourf(xx, yy, Z.reshape(xx.shape), cmap = 'viridis', levels = levels)
    plt.colorbar(cset, ax=ax)
    if cmap: ax.scatter(xs, ys, c=y, cmap=cmap)
